...
The installer selects an available host port between `8010` and `8099` (container port `8000`). Copy `.env.example` to `.env` and provide a Cloudflare Tunnel token only when the tunnel has been created.
